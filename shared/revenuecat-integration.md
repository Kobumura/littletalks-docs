...
## RevenueCat Integration - Live Subscription Checking

### How RevenueCat Status is Checked
The app checks RevenueCat subscription status **live** on every app launch/login:

```javascript
// From userContext.js - grabUser() function
const rcResp = await Purchases.logIn(user?.id);
let { customerInfo } = rcResp;
setCustomerInfo(customerInfo);

if (customerInfo?.entitlements?.active && 
    Object.entries(customerInfo?.entitlements?.active).length) {
  // User has active subscription
  const activeSubscriptionIdentifier = activeSubscriptionsIdentifiers[0];
  setRevenueCatEntitlement(activeSubscription);
  
  // Fetch limits from backend based on subscription
  const limits = await getUserLimits(
    `revenueCatEntitlement=${activeSubscriptionIdentifier}`
  );
} else {
  // No active subscription - use free tier
  setRevenueCatEntitlement(null);
  setRemainingFreeTierBenefits(user.remainingFreeTierBenefits);
}
```

**Key Points:**
- No webhook integration - app pulls latest status from RevenueCat SDK
- Backend trusts the `revenueCatEntitlement` passed from mobile app
- Subscription changes only reflect when user opens the app
- Payment grace periods are handled by App Store/Google Play
...
// Note: Maximum 20 characters total due to phone number pool limitation
```
...
### Frontend Current Usage Tracking
The frontend pre-calculates usage for subscription users:

```javascript
// Only count entities from last 30 days for subscription users
let filteredCharacters = user.allMyCharacters?.filter((character) => {
  const createdAt = new Date(character.createdAt);
  const thirtyDaysAgo = new Date();
  thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
  return (
    createdAt >= thirtyDaysAgo &&
    character.billingSource === "subscription"
  );
});
setCurrentCharacter(filteredCharacters?.length || 0);
```

### Frontend (Mobile App)
1. **Live RevenueCat checking** on every app launch via SDK
2. **No blocking validation** on navigation - better UX
3. **userContext** provides limits and current usage (pre-calculated)
4. **Error handling** navigates to subscription screen on limit reached
5. **UI Test Mode** - Special handling with mock data when `isUITest` launch arg is true
...
- Messages reset at the beginning of each calendar month (likely UTC timezone)
- Yearly billing is a discount option but limits remain monthly (not 1200 messages upfront)
- Characters and recipients are cumulative (don't reset)
- **Hard limit**: Maximum 20 characters per deployment due to phone number pool (via Twilio)
...
## Error Responses & Frontend Handling

### Backend Error Messages
- Characters: `throw new Error("MAX_CHARACTERS_REACHED")`
- Messages: `throw new Error("MAX_MESSAGES_REACHED")`
- Recipients: `throw new Error("MAX_SENDERS_REACHED")` *(should be MAX_RECIPIENTS_REACHED)*

### Frontend Handling
- Shows appropriate error toast/alert
- May navigate to subscription/upgrade screen
- Recipients screen pre-checks count before API call to improve UX

## Known Issues & Future Improvements

### 1. Subscription Downgrade Handling (Ticket: LTD-113)
When users cancel/downgrade and exceed new limits, the system needs to handle existing content gracefully.

### 2. Error Message Inconsistency
Backend throws `MAX_SENDERS_REACHED` but should be `MAX_RECIPIENTS_REACHED` for consistency.

### 3. RevenueCat Webhook Integration
Currently relies on app launches to check subscription status. Webhook integration would provide:
- Real-time subscription updates
- Immediate downgrade/upgrade reflection
- Better handling of payment failures

### 4. Edge Cases Not Yet Addressed
- RevenueCat service downtime (could cache last known good state)
- Timezone considerations for monthly resets (assumes UTC)
- Grace period handling (currently managed by app stores)